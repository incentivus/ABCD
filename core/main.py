...
async def main():
    ...
    await BOB.update_balance("Margin + Guarantee")
    await CAROL.update_balance("Cadillac's License")
    ...
    await notify_finish_commitment()
    await wait_until_next_interrupt()
    ...
    await ALICE.update_balance("Nothing")
    await BOB.update_balance("Margin + Guarantee + Locked Premium")

    await ALICE.new_message("Locked premium for Bob.")
    ...
    await BOB.update_balance("Locked Premium")
    # await BOB.new_message("Initialized payment channel on his side.")
    await BOB.new_message("Locked margin and guarantee for Alice.")

    # await unlock_fundings()
    ...
    # await ALICE.new_message("Locked Bob's guarantee.")
    ...
    await BOB.update_balance("Premium")
    await BOB.new_message("Got the premium from Alice.")
    ...
    await CAROL.update_balance("Cadillac's License + Locked Principal")
    await BOB.new_message("Principal is fulfilled and locked for Alice")
    ...
    await CAROL.update_balance("Locked Principal")
    await ALICE.update_balance("Locked Cadillac's License")

    await CAROL.new_message("Cadillac's license locked for Alice.")
    ...
    await ask_bob_unlocks()
    while True:
        await asyncio.sleep(1)
        if asyncState.bob_unlocks == 'P':
            continue
        else:
            break
    if asyncState.bob_unlocks == 'F':
    ...
    await BOB.update_balance("Payback + Premium + Guarantee")

    await BOB.new_message("Unlocked Alice's payback.")
    ...
    await ALICE.update_balance("Cadillac's License")

    await ALICE.new_message("Unlocked Cadillac's license.")
    ...
    await CAROL.new_message("Got the Cadillac's cost.", end=True)
