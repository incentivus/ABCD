...
# from litecoinutils.transactions import Transaction
...
class Exchange(Participant):
    ...
    def make_HTLC(self,
                  ...
                  ) -> Tuple[ltctrans.Transaction, UTXO]:
        amount_to_send = utxo.value - fee
        txout_script = HTLC_script(
            network="ltc-tst",
            sender_address=self.pubkey_hash(network="ltc-tst"),
            recipient_address=recipient_pubkeyhash,
            secret_hash=bob_principal_utxo.redeem_script.script[11],
            locktime=locktime,
        )

        txin = utxo.create_tx_in(network="ltc-tst")
        txout = ltctrans.TxOutput(
            amount_to_send,
            txout_script
        )
        self.HTLC_tx = ltctrans.Transaction([txin], [txout])
        self.sign_p2pkh(self.HTLC_tx, 0, network="ltc-tst")
        new_utxo = UTXO(
            network="ltc-tst",
    ...
    def commit_HTLC(self):
        self.HTLC_tx = ltctrans.Transaction.copy(self.HTLC_tx)
